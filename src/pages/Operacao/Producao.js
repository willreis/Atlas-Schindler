...
export default function Producao() {
    ...
    const [getProcesso, setGetProcesso] = useState([]);
    ...
    useEffect(() => {
        ...
        Api.get(`${urlProcesso}`)
            .then((response) => {
                setGetProcesso(
                    response.data.map((getProcesso) => {
                        console.log("Retorne apenas o Id de Processo:", getProcesso.processoId);
                        return {
                            processoId: getProcesso.processoId,
                        }
                    }))
            })
            .catch((error) => {
                alert("Erro no Get de Processo: ", error);
            })
    }, []);
